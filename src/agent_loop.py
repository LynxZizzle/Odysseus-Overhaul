...
def _load_mcp_disabled_map() -> Dict[str, set]:
    ...
    try:
        ...
        if isinstance(builtin_disabled, dict):
            for _sid, _names in builtin_disabled.items():
                if _names:
                    disabled_map.setdefault(_sid, set()).update(_names)
    except Exception:
        pass
    return disabled_map
...
def _build_system_prompt(
    ...
    try:
        ...
        _ov_sig = ""
    # Per-server MCP disabled-tool sets must also bust the cache: without
    # this, toggling an individual MCP tool off/on in the admin panel updates
    # the live function-calling schema list (get_all_openai_schemas, called
    # fresh every request) but NOT this cached prompt text — so the model
    # could keep seeing a disabled tool *described* in its system prompt
    # while it's no longer actually callable, producing a confusing
    # "tool not found" failure that looks like the toggle didn't take effect
    # until a reconnect happens to rebuild everything from scratch.
    _mcp_dis_key = frozenset(
        (sid, frozenset(names)) for sid, names in (mcp_disabled_map or {}).items()
    )
    cache_key = (frozenset(disabled_tools or []), bool(mcp_mgr), needs_admin, _rt_key, compact, _ov_sig, owner, suppress_local_context, suppress_skills, _mcp_dis_key)
